...
const AllPackages = () => {
    ...
    try {
      ...
      let url =
        filter === "offer" //offer
          ...
          : `http://localhost:8000/api/package/get-packages?searchTerm=${search}`; //all
  ...
  const onShowMoreSClick = async () => {
    ...
    let url =
      filter === "offer" //offer
        ? `http://localhost:8000/api/package/get-packages?searchTerm=${search}&offer=true&startIndex=${startIndex}`
        : filter === "latest" //latest
        ? `http://localhost:8000/api/package/get-packages?searchTerm=${search}&sort=createdAt&startIndex=${startIndex}`
        : filter === "top" //top rated
        ? `http://localhost:8000/api/package/get-packages?searchTerm=${search}&sort=packageRating&startIndex=${startIndex}`
        : `http://localhost:8000/api/package/get-packages?searchTerm=${search}&startIndex=${startIndex}`; //all
    const res = await fetch(url);
    const data = await res.json();
